						...
						<ul class="list-group">
								...
								<div class="d-flex flex-column">
									...
									<div class="form-group">
										...
										<textarea class="form-control" rows="10" cols="200"
											id="content" name="content" placeholder="내용을 입력해주세요"
							...
							<c:if test="${empty reply.bno=='False'}">
								<li
									...
									<div class="d-flex flex-column">
										<div class="form-group">
											<label for="title">답글번호:</label> <input type="text"
												class="form-control" id="bno" name="bno"
												value="re:${reply.rno}" readonly>
										</div>
										<div class="form-group">
											<label for="title">답글 제목:</label> <input type="text"
												class="form-control" id="title" placeholder="제목을 입력해주세요"
												name="title" value="re:${board.title}" readonly>
										</div>
										<div class="form-group">
											<label for="writer">작성자:</label> <input type="text"
												class="form-control" id="writer" name="writer"
												value="${reply.writer}" readonly>
										</div>
										<div class="form-group">
											<label for="content">내용:</label>
											<textarea class="form-control" rows="10" cols="200"
												id="content" name="content" readonly="readonly">${reply.content}</textarea>
										</div>
									</div>
								</li>
							</c:if>
...
<script type="text/javascript">
	...
	$(".filedown").click(
			function() {
				...
				alert("fno:" + fno + "원본:" + $(this).attr('ofile') + "실제:"
						+ $(this).attr('sfile'));
				location.href = "/qna/download/" + fno;
			});
	/* $("#btnUpdate").click(function() {
		if (confirm("정말 수정할까요?")) {
			location.href = "/qna/update/${board.bno}"
		}
	}); */
	...
	$("#btnList")
			.click(
					function() {
						if (confirm("정말 이동할까요?")) {
							location.href = "/qna/list?pageNum=${pageNum}&field=${field}&word=${word}"
						}
					});
</script>